...
function formatMonth(dateISO: string): string {
  try { return new Date(dateISO).toLocaleDateString(undefined, { year: "numeric", month: "short" }); }
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-neutral-50 text-neutral-900 dark:from-neutral-950 dark:to-neutral-900 dark:text-neutral-50">
      ...
      <header className="sticky top-0 z-40 border-b bg-white/70 backdrop-blur dark:border-neutral-900 dark:bg-neutral-950/60">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <nav className="hidden gap-6 sm:flex">
            ...
            <a href="#projects" onClick={(e) => { e.preventDefault(); navigate("home"); setTimeout(() => document.getElementById("projects")?.scrollIntoView({ behavior: "smooth" }), 0); }} className="text-sm">Projects</a>
            <a href="#certs" onClick={(e) => { e.preventDefault(); navigate("home"); setTimeout(() => document.getElementById("certs")?.scrollIntoView({ behavior: "smooth" }), 0); }} className="text-sm">Certifications</a>
            <a href="#blog" onClick={(e) => { e.preventDefault(); navigate("blog"); }} className="text-sm">Blog</a>
            <a href="#contact" onClick={(e) => { e.preventDefault(); navigate("home"); setTimeout(() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" }), 0); }} className="text-sm">Contact</a>
          </nav>
        ...
        <main>
          <section className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-4 py-14 text-center sm:py-20">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              <Button as="a" href={`mailto:${PROFILE.email}`}><Mail className="h-4 w-4" /> Contact me</Button>
              <Button onClick={() => document.getElementById("services")?.scrollIntoView({ behavior: "smooth" })}>Let's collaborate <ArrowRight className="h-4 w-4" /></Button>
              <a href={PROFILE.socials.github} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm underline-offset-4 hover:underline"><Github className="h-4 w-4" /> GitHub</a>
              <a href={PROFILE.socials.linkedin} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm underline-offset-4 hover:underline"><Linkedin className="h-4 w-4" /> LinkedIn</a>
            </div>
          </section>

          <BlogSpotlight posts={posts} onOpen={(p) => navigate("post", { slug: p.slug })} onViewAll={() => navigate("blog")} />
            ...
            <motion.div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={listContainer}>
              {SKILLS.map(({ icon: Icon, label, notes }) => (
                <motion.div key={label} className="rounded-2xl border p-4 transition hover:shadow dark:border-neutral-800" variants={listItem}>
                  <div className="mb-1 inline-flex items-center gap-2 text-sm font-semibold"><Icon className="h-5 w-5" /> {label}</div>
          ...
          <Section id="services" title="Let’s collaborate" kicker="Open to new data challenges">
            <div className="rounded-2xl border p-6 dark:border-neutral-800">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {SKILLS.map((s) => <Badge key={s.label}>{s.label}</Badge>)}
              </div>
              <div className="mt-4">
            ...
            <motion.div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={listContainer}>
              {SELECTED_WORK.map((c) => (
                <motion.div key={c.title} className="rounded-2xl border p-4 dark:border-neutral-800" variants={listItem}>
                  ...
                  <div className="mb-4 flex flex-wrap gap-2">{c.stack.map((s) => <Badge key={s}>{s}</Badge>)}</div>
                  {c.link && <a href={c.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm underline-offset-4 hover:underline">View details <ExternalLink className="h-4 w-4" /></a>}
                </motion.div>
              ))}
            </motion.div>
          </Section>
            ...
            <motion.div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={listContainer}>
              {SIDE_PROJECTS.map((p) => (
                <motion.div key={p.title} className="rounded-2xl border p-4 dark:border-neutral-800" variants={listItem}>
                  ...
                  {p.link && <a href={p.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm underline-offset-4 hover:underline">View on GitHub <ExternalLink className="h-4 w-4" /></a>}
                </motion.div>
              ))}
            </motion.div>
          </Section>
              ...
              {CERTS.map((c) => (
                <motion.div key={c.name} className="rounded-2xl border p-5 dark:border-neutral-800" variants={listItem}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge><Award className="mr-1 h-3 w-3" /> {c.issuer}</Badge>
                  </div>
                </motion.div>
              ))}
            </motion.div>
          ...
          <Section id="latest" title="Latest writing" kicker="From the blog">
            <motion.div className="grid gap-4 md:grid-cols-3" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={listContainer}>
              {latestPosts.map((p) => (
                <motion.article key={p.id} className="rounded-2xl border p-4 transition hover:shadow dark:border-neutral-800" variants={listItem}>
                  <h3 className="mb-2 text-lg font-semibold">{p.title}</h3>
                  <div className="mb-3 flex flex-wrap gap-2">{(p.tags || []).map((t) => <Badge key={t}>{t}</Badge>)}</div>
                  <Button onClick={() => navigate("post", { slug: p.slug })}>Read <ArrowRight className="h-4 w-4" /></Button>
                </motion.article>
              ))}
            </motion.div>
            <div className="mt-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="rounded-2xl border p-6 dark:border-neutral-800">
                ...
                <div className="flex flex-col gap-3 text-sm">
                  <a href={`mailto:${PROFILE.email}`} className="inline-flex items-center gap-2 underline-offset-4 hover:underline"><Mail className="h-4 w-4" /> {PROFILE.email}</a>
      ...
      {route.name === "blog" && (
        <main className="mx-auto max-w-6xl px-4 py-12 sm:py-16">
          ...
          <BlogList posts={posts} onOpen={(p) => navigate("post", { slug: p.slug })} query={query} setQuery={setQuery} loading={loading} />
        </main>
      )}
